...
const AppointmentBookingApp = () => {
  ...
  return (
    <div>
      ...
      {/* Floating Chat Panel */}
      {openFloatingChat && (
        <div className="fixed bottom-20 right-6 z-50 w-[22rem] md:w-[26rem] bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-200">
          <div className="flex items-center justify-between px-4 py-3 border-b bg-white/95 backdrop-blur">
            <div className="font-medium text-slate-800">Talk to Assistant</div>
            <div className="flex items-center space-x-2">
              <button
                onClick={() => setOpenFloatingChat(false)}
                aria-label="Close chat"
                className="text-gray-500 hover:text-gray-700 px-2 py-1 rounded-md hover:bg-slate-100"
              >
                ×
              </button>
            </div>
          </div>
          <div className="h-[68vh] max-h-[620px] min-h-[460px]">
            <ChatFloating />
          </div>
        </div>
      )}
